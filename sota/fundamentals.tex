...
\par Following up on the previously described systems, it is possible to infer that a DL approach can be divided in three implementation choices: the methods of 1) face detection and 2) face recognition, and 3) the loss function necessary for training\footnote{As this dissertation specifically concentrates on the face recognition domain, the loss function pertains exclusively to the training of that stage.}. Ganidisastra and Bandung~\autocite{ganidisastraIncrementalTrainingDeep2021} and SMOWL~\autocite{labayenOnlineStudentAuthentication2021} both utilize Facenet trained with triplet loss for face recognition. Therefore, it would be interesting to investigate the effects of a lightweight network (prefiro ainda não referir qual/quais) with fewer parameters than Facenet on performance. Furthermore, considering the drawbacks associated with triplet loss, examining the impact of a well-established loss function, like ArcFace, which encourages intra-class compactness and inter-class distance, would also be noteworthy.
\end{document}