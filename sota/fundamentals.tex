...
\par Furthermore, face alignment (or landmark localization), can be accomplished through two categories of methods: landmark-based and landmark-free.
...
\par As can be seen from the previous section, this step in the face recognition process can be accomplished, very sporadically, through standalone methods that process the detected face from the previous stage, but generally joint detection and alignment methods (and sometimes even face representation) is the optimal choice \autocite{changFacePoseNetMakingCase2017}, as previously mentioned in the \hyperref[mt learning]{multi-task learning} definition.
